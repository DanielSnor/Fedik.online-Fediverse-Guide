...
// Fedík.online — průvodce Fediverse. Vše běží v prohlížeči nad statickými daty
// (taxonomy.js, apps.js, tools.js, glossary.js, links.js, instances.*.js + data/*.json).
//
// Pohledy: Začínáme (onboarding), Aplikace, Instance, Nástroje, Slovníček, Statistiky,
// Odkazy, O Fedíku, Vyhledávání. Stav v URL hash, i18n cs/en, mobilní drawer.
// Vzniklo z frontendu Sloník.online; katalog účtů a postů byl odstraněn 25. 9. 2026.
...
(function () {
  ...
  var STRINGS = {
    cs: {
      ...
      menu_show: 'Menu',
      menu_hide: 'Zavřít',
      // ─── Sdílené popisky (vyhledat / řadit) ───
  ...
  function setLang(next) {
    ...
    applyI18n();
    applyView();   // zachová aktuální pohled a překreslí ho v novém jazyce
  ...
  // ---------- DOM ----------
  var sidebarEl, sidebarToggleEl, modalEl, aboutViewEl, aboutNavEl,
      searchViewEl, searchQEl, searchMetaEl, searchResultsEl, searchNavEl, searchTabsEl,
  ...
  function placeSliceTabs() {
    var slices = [searchTabsEl, instanceTabsEl, linksTabsEl].filter(Boolean);
  ...
  // Klik na hlavičkový obrázek = návrat na landing (Začínáme, krok 1).
  function bindHeroHome() {
    var hero = document.getElementById('hero-home');
    if (hero) hero.addEventListener('click', goHome);
  }
  function goHome() {
    view = 'start'; startStep = 1; startChosenInstance = null; pendingChosenInstHost = '';
    applyView();
    writeHash();
    window.scrollTo(0, 0);
  }
  ...
  function parseHash() {
    ...
    hash.split('&').forEach(function (pair) {
      // Holé tokeny bez "=" (např. #instance) → přepínač pohledu.
