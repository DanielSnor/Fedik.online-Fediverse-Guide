#!/usr/bin/env bash
# ============================================================
# Fedík: Sync Test → Prod (na serveru)
# ============================================================
# Promote kódu a web assetů z TEST do PROD instance. Spouští se přímo na
# serveru (v kontejneru) — rsync běží lokálně bez SSH. Přenese KÓD a web
# assety (vč. obrázků), ale NIKDY:
#   config.env, scripts/deploy.env, data/, logs/, web/data/ (data JSONy)
#   a (defaultně) ani config/ — prod má vlastní.
...
#   ./scripts/sync_test_to_prod.sh --with-config  # i config/ (crontab)
# ============================================================
set -euo pipefail
...
rsync -av $DRY \
  ...
  --exclude='data/' --exclude='logs/' --exclude='web/data/' \
  $CONFIG_EXCLUDE \
  "$TEST_DIR/" "$PROD_DIR/"
...
echo -e "Nedotčeno v prod: ${CYAN}config.env, config/, data/, web/data/, logs/${NC}"
