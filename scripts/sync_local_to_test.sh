...
DIR="${FEDIK_REMOTE_DIR:?FEDIK_REMOTE_DIR není v deploy.env}"
...
# 5a) web/ root assety (HTML/JS/CSS/MD + obrázky) — BEZ *.json (data serveru) a BEZ podadresářů
echo -e "${CYAN}== web/ root assety (bez *.json a podadresářů) ==${NC}"
rs --exclude='img/' --exclude='data/' \
...
# 5b) web/img/ — všechny obrázky
echo -e "${CYAN}== web/img/ ==${NC}"
...
echo -e "Nedotčeno na serveru: ${CYAN}config.env, data/, web/data/, logs/${NC}"
